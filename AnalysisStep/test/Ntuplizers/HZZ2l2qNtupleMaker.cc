...
//Z1tau21.push_back(thisZ1tau21);
  ...
  for (unsigned int i=0; i<leptons.size(); ++i){
   ...
   // SIP[-2]             = userdatahelpers::getUserFloat(leptons[i],"SIP"); //FIXME aloke
   // passIsoPreFSR      = passIsoPreFSR&&(userdatahelpers::getUserFloat(leptons[i],"combRelIsoPF")<LeptonIsoHelper::isoCut(leptons[i])); //FIXME aloke
    ...
    if (theChannel!=ZL) {
	cout<<labels[i]+"combRelIsoPFFSRCorr"<<endl;
      //combRelIsoPF[i-2]    = cand.userFloat(labels[i]+"combRelIsoPFFSRCorr"); //FIXME aloke // Note: the
     // assert(SIP[i-2] == cand.userFloat(labels[i]+"SIP"));//FIXME aloke  // Check that I don't mess up with labels[] and leptons[]
    } else {
      //FIXME: at the moment,  FSR-corrected iso is not computed for Z+L CRs
      cout<<"aloke"<<endl;
      //combRelIsoPF[i-2]    = userdatahelpers::getUserFloat(leptons[i],"combRelIsoPF");// FIXME aloke
    }
    ...
    //LepSIP  .push_back( SIP[i-2] ); //FIXME aloke
