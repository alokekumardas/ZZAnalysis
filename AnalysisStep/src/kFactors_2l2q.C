 float kfactor_qqZZ_qcd_dPhi_2l2q(float GENabsdPhiZZ, int finalState)
{
...
float xsec_qqZZ_qcd_M_2l2q(float GenMassZZ, int finalState, int order){ // Order: 0=LO, 1=NLO, 2=NNLO
  const int nbins=20;
...
float kfactor_qqZZ_qcd_M_2l2q(float GENmassZZ, int finalState, int order) // Order: 1=NLO, 2=NNLO
{
  // finalState=1 : 4e/4mu/4tau
  // finalState=2 : 2e2mu/2mutau/2e2tau

  float k=1;
  float xsec_lo = xsec_qqZZ_qcd_M_2l2q(GENmassZZ, finalState, 0);
  float xsec = xsec_qqZZ_qcd_M_2l2q(GENmassZZ, finalState, order);
  k = xsec/xsec_lo;
  return k;
}


float kfactor_qqZZ_qcd_Pt_2l2q(float GENpTZZ, int finalState)
{
